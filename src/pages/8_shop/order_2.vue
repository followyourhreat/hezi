<template>
  <div>
    ...
    <Modal v-model="modal2" :title="modelTitle" :mask-closable="false">
      <Form ref="form2" :model="formData2" :label-width="100">
        <FormItem label="收货人手机">{{formData2.address.phone}}</FormItem>
        <FormItem label="收货人地址">{{formData2.address.address}}</FormItem>
        <FormItem label="收货人姓名">{{formData2.address.realname}}</FormItem>
        <FormItem label="快递订单号">
          <Input v-model="formData2.express_code" placeholder="请输入快递订单号"></Input>
        </FormItem>
      </Form>
      <div slot="footer">
...
export default {
  ...
  methods: {
    ...
    handleStatus(item) {
      this.modal2 = true;
      this.formData2 = Object.assign(this.formData2, item);
      this.formData2.address = item.address
        ? item.address
        : { phone: "", address: "", realname: "" };
    },
    handleSubmit2() {
      this.$post("mallorder/delivery", {
        express_code: this.formData2.express_code
      }).then(res => {
        this.handleSearch();
      });
    }
